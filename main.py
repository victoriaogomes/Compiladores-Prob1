...
for i in range(len(lista_arquivos)):  # Para cada arquivo presente no folder input
    ...
    error_list = analyzer.output_list
    if not analyzer.error:
        semantic = semantic_analyzer.SemanticAnalyzer(analyzer.global_table, ast, error_list)
        error_list = semantic.analyze()
        print('--- Conclusão da análise semântica do arquivo', lista_arquivos[i])
    else:
        print('--- Devido a presença de erros sintáticos, a análise semântica do arquivo', lista_arquivos[i], 'não foi realizada')
    filesManager.write_symbol_table(error_list.get_tokens(), lista_arquivos[i])
